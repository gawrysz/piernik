...
run_piernik() {
    ...
    if [ $scaling == flood ]; then
        ...
        case $problem in
            ...
            crtest)
                local xmul=512
                run_strong_weak_scaling $scaling $threads $nx "$mpirun_cmd" $max_mem $xmul | grep "C1-cycles" | awk '{if (NR==1) printf(" %8.4f %8.4f ", $5, $8)}'
...
process_output() {
    ...
    case $problem in
        sedov)
            ...
            ;;
        crtest)
            grep "C1-cycles" $dir/_stdout_ | awk '{if (NR==1) printf("%d %8.4f %8.4f ", '$core', $5, $8)}'
            awk '/Spent/ { printf("%s\n", $5) }' $dir/*log
            ;;
        maclaurin)
